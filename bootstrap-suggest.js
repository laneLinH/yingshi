    ...
    function refreshDropMenu($input, data, options) {
        ...
        for (i = 0; i < len; i++) {
            ...
            for (j in data.value[i]) {
                ...
                tds.push('<td data-name="', j, '"><a style="cursor: pointer;" title="', data.value[i][j], '">', data.value[i][j], '</a></td>');
            }
        ...
        init: function (options) {
            ...
            return self.each(function () {
                ...
                $input.on('keydown', function (event) {
                    ...
                    if (event.keyCode === options.keyDown || event.keyCode === options.keyUp || event.keyCode === options.keyEnter || event.keyCode == 9) {
                    ...
                    //若输入框值没有改变或变为空则返回， 否则重置选中的值
                    if ($.trim(word) !== '' && word === $(this).attr('alt')) {
                        return;
                    }else if(handleSuggestValChange){
                      handleSuggestValChange($(this));
                    }
